...
pub struct CAddEntity {
    ...
    /// Entity position
    pub pos: DVec3,
    /// Entity velocity (blocks per tick)
    pub vel: DVec3,
...
pub fn write_lp_vec3(writer: &mut impl std::io::Write, vel: DVec3) -> std::io::Result<()> {
    ...
    if chessboard_length < ABS_MIN_VALUE {
        ...
        let xn = pack_component(vel.x / scale as f64);
        let yn = pack_component(vel.y / scale as f64);
        let zn = pack_component(vel.z / scale as f64);
